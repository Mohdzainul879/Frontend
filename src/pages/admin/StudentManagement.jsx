      ...
      <main className="flex-1 p-6 space-y-6 overflow-auto">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-500 text-sm">{mockStudents.length} students enrolled</p>
          </div>
          <div className="flex gap-2">
            <span className="bg-red-100 text-red-700 text-xs font-medium px-3 py-1.5 rounded-lg flex items-center gap-1">
              <FiAlertTriangle size={13} />
              {mockStudents.filter(s => s.accountabilityScore < 50).length} flagged
            </span>
          </div>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {students.map(s => (
                  <tr key={s.id} className={`hover:bg-gray-50 transition-colors ${s.accountabilityScore < 50 ? 'bg-red-50' : ''}`}>
